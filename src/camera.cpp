...
void Camera::PanUp(float amt)
{
    TranslateAlongUp(amt);
}
void Camera::PanRight(float amt)
{
    TranslateAlongRight(amt);
}
void Camera::ZoomForward(float amt)
{
    TranslateAlongLook(amt);
}
void Camera::RotateAboutUpSphere(float deg)
{
    glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(deg), up);
    eye-=ref;
    eye = glm::vec3(rotation*glm::vec4(eye,0));
    eye+=ref;
    RecomputeAttributes();
}
void Camera::RotateAboutRightSphere(float deg)
{
    glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(deg), right);
    eye-=ref;
    eye = glm::vec3(rotation*glm::vec4(eye,0));
